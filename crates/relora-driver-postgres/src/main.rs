...
use relora_driver_postgres::PostgresDriver;

#[derive(Debug, Parser)]
#[command(
    name = "relora-driver-postgres",
    version,
    about = "Relora external PostgreSQL driver sidecar."
)]
struct Cli {
    #[arg(long)]
    url: String,
...
fn main() -> Result<()> {
    let cli = Cli::parse();
    let mut driver = PostgresDriver::connect(&cli.url)?;
