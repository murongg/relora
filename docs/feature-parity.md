...
| Tree presentation | `Done` | `Done` | `Done` | PostgreSQL keeps a native `database -> schema -> object` hierarchy. MySQL and SQLite collapse duplicate schema aliases so the tree matches their dialect semantics more closely. |
...
- [ ] Expand MySQL / SQLite object-kind coverage where the drivers can expose richer metadata
